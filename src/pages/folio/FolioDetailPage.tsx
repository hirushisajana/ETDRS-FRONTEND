...
export default function FolioDetailPage() {
  ...
  const handleViewSigned = async () => {
    try {
      ...
      alert('Failed to load signed folio');
    }
  };
    ...
    <div className="p-6 lg:p-8">
      ...
      <div className="flex items-center justify-between mb-6 no-print">
        ...
        <div className="flex items-center gap-2">
          ...
          {folio.hasSignedFolio && hasRole('FOLIO_USER', 'COUNTER_USER', 'REGISTRY_ADMIN', 'IT_ADMIN', 'SUPER_ADMIN') && (
            <button
              onClick={handleViewSigned}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-maroon-700 bg-maroon-50 border border-maroon-200 rounded-xl hover:bg-maroon-100 transition-colors"
            >
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z" />
              </svg>
