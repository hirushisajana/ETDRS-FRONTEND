    ...
    <div className="p-6 lg:p-8 max-w-[1400px]">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statCards.map(({ label, value, accent, icon, iconBg }) => (
          <div
            key={label}
            className={`bg-white rounded-3xl border border-slate-200 border-l-[3px] ${accent} p-5 shadow-sm hover:shadow-md transition-shadow`}
          >
      ...
      <div className="bg-white rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">Pending Queue</h3>
            <p className="text-[11px] text-slate-400 mt-0.5">Folios in daybook number order — click to enter data</p>
          </div>
        </div>
        {pendingList.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="text-sm font-medium text-slate-500">No pending folios</p>
            <p className="text-xs text-slate-400 mt-1">All folio entries have been processed</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-50">
                {pendingList.map((folio, idx) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-600">{folio.trustCategory}</td>
