...
import QuarterlyUpdateForm from '../pages/counter/QuarterlyUpdateForm';
...
const renderForm = (onSuccess = vi.fn()) =>
  render(<QuarterlyUpdateForm onSuccess={onSuccess} />);

describe('QuarterlyUpdateForm lookup', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('shows an eligibility message instead of not found for non-REGISTERED deeds', async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/is not eligible for a quarterly update \(current status: PENDING\)/i),
      ).toBeInTheDocument();
  ...
  it('submits the update with client details prefilled from the original', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'Create Quarterly Update' }));
